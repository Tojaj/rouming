...
public class Rouming {
	...
	public static String string_cleanup(String str) {
	...
	public static long to_size_in_kb(String size) {
		if (size.endsWith("kB")) {
			return to_long(size.replace("kB", ""));
		} else {
			return -1;
		}
	}
	
	public static URL to_url(String str) {
		try {
			return new URL(str);
	...
	public static URL to_pic_url(String str) {
		String splited[] = str.split("file=");
		
		String img_url = RoumingConf.ROUMING_IMG_URL + splited[splited.length-1];
	...
	public static long to_unix_time(String str) {
		...
		if (str.matches("[0-9]{1,2}:[0-9]{1,2}")) {
			...
			String current_day = new SimpleDateFormat("dd.MM.yyyy", locale).format(new Date());
			str = current_day + "-" + str;
		} else {
		...
		try {
			...
			Log.e(TAG, "Cannot convert date string \""+str+"\" to milliseconds");
	...
	public static ArrayList<RoumingPictureHref> getRoumingPictureHrefs(Context context) {
	...
	public static ArrayList<RoumingPictureHref> parseRoumingHtml(String content) {
		...
		while (m.find()) { // Find each match in turn; String can't do this.
		     ...
		     pic_href.name = string_cleanup(p_name);
		     pic_href.time = string_cleanup(p_time);
		     pic_href.unixtime = to_unix_time(string_cleanup(p_time));
		     pic_href.comments = to_long(string_cleanup(p_comments));
		     pic_href.likes = to_long(string_cleanup(p_likes));
		     pic_href.dislikes = to_long(string_cleanup(p_dislikes));
		     pic_href.size = to_size_in_kb(string_cleanup(p_sizekb));
		     pic_href.url = to_url(string_cleanup(p_url));
		     pic_href.pic_url = to_pic_url(string_cleanup(p_url));
